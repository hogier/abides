...
class TradingAgent(FinancialAgent):
  ...
  def placeLimitOrder (self, symbol, quantity, is_buy_order, limit_price, order_id=None, ignore_risk = True, tag = None):
    ...
    if quantity > 0:
      ...
      # Log this activity.
      if self.log_orders: self.logEvent('ORDER_SUBMITTED', js.dump(order, strip_privates=True, strict=True))
    else:
  ...
  def cancelOrder (self, order):
    ...
    # Log this activity.
    if self.log_orders: self.logEvent('CANCEL_SUBMITTED', js.dump(order, strip_privates=True, strict=True))
  ...
  def modifyOrder (self, order, newOrder):
    ...
    # Log this activity.
    if self.log_orders: self.logEvent('MODIFY_ORDER', js.dump(order, strip_privates=True, strict=True))
  ...
  def orderExecuted (self, order):
    ...
    # Log this activity.
    if self.log_orders: self.logEvent('ORDER_EXECUTED', js.dump(order, strip_privates=True, strict=True))
  ...
  def orderAccepted (self, order):
    ...
    # Log this activity.
    if self.log_orders: self.logEvent('ORDER_ACCEPTED', js.dump(order, strip_privates=True, strict=True))
  ...
  def orderCancelled (self, order):
    ...
    # Log this activity.
    if self.log_orders: self.logEvent('ORDER_CANCELLED', js.dump(order, strip_privates=True, strict=True))

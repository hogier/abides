# HERD-7 (Herd Behavior Simulation Configuration):
# - 1     Exchange Agent
# - 1     POV Market Maker Agent
# - 1     Herd Master Agent
# - 120    Herd Slave Agent
# - 100   Value Agent
# - 500   Momentum Agent
# - 1000  ZI Agent
# - 500   HBL Agent
...
# Mixed strategy, 2 hours, 120 slaves, 100 runs, 25 seconds max delay
...
parser = argparse.ArgumentParser(description='Detailed options for HERD07 config.')
...
kernel = Kernel("HERD07 Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32,
                                                                                                  dtype='uint64')))

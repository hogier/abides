# HERD-8 (Herd Behavior Simulation Configuration):
# - 1     Exchange Agent
...
# Mixed strategy, 2 hours, 30 slaves, 100 runs, 40 seconds max delay, 60 future window, size 2
...
parser = argparse.ArgumentParser(description='Detailed options for HERD08 config.')
...
kernel = Kernel("HERD08 Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32,
                                                                                                  dtype='uint64')))

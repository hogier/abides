# HERD10 (Herd Behavior Simulation Configuration):
# - 1     Exchange Agent
...
# Mixed strategy, 2 hours, 60 slaves, 100 runs, accepted
...
############################################### GENERAL CONFIG #########################################################
CONFIG_NAME = 'HERD10'
parser = argparse.ArgumentParser(description=f'Detailed options for {CONFIG_NAME} config.')
...
historical_date = pd.to_datetime('2021-01-30')
symbol = 'ABM'
...
slaves_arrival = 'uniform'
send_signal = 'accepted'
...
kernel = Kernel(f"{CONFIG_NAME} Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32,
                                                                                                  dtype='uint64')))
